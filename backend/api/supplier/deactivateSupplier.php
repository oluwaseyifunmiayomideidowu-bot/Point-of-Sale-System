...
<?php
    ...
    if ($supplier['status'] === 'Inactive') {
        sendResponse(false, 'Supplier account is already inactive.', null, 409);
    }

    // Deactivate supplier
    $result = deactivateSupplier($conn, $supplierId);

    if (!$result) {
        sendResponse(false, 'Unable to deactivate supplier.', null, 500);
    }

    sendResponse(true, 'Supplier deactivated successfully.', null, 200);
